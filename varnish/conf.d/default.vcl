...
sub vcl_recv {
    ...
    if (!true || req.http.Authorization ||req.request !~ "^(GET|HEAD|OPTIONS)$" || req.http.Cookie ~ "varnish_bypass=1") {
        if (req.url ~ "^(/ch/media/|/ch/skin/|/media/|/ch/js/|/skin/|/js/|/ch/|/uk/|/)(?:(?:index|litespeed)\.php/)?b4ckend") {
            set req.backend = admin;
        }

        std.log("----- ----- Detected Admin-URL: " + req.url);
        std.log("----- ----- ----- ----- return pipe");
        return (pipe);
    }


      std.log("----- ----- ----- ----- continue recv");


    set req.url = regsuball(req.url, "([^:])//+", "\1/");
    if (req.url ~ "^(/ch/media/|/ch/skin/|/media/|/ch/js/|/skin/|/js/|/ch/|/uk/|/)(?:(?:index|litespeed)\.php/)?") {
        ...
        if (true &&
        req.url ~ ".*\.(?:css|js|jpe?g|png|gif|ico|swf|svg)(?=\?|&|$)") {
        ...
        if (req.url ~ "^(/ch/media/|/ch/skin/|/media/|/ch/js/|/skin/|/js/|/ch/|/uk/|/)(?:(?:index|litespeed)\.php/)?(?:b4ckend|api|cron\.php|search)" ||
...
sub vcl_hash {
    ...
    std.log("----- STATE: [ hash ] with url: ");
    std.log("URL: " + req.url);
    std.log("Cookie: " + req.http.Cookie);


    if (true && req.http.X-Varnish-Static) {
        std.log("----- HASH req.url: " + req.url);
        hash_data(req.url);
    if (req.http.Accept-Encoding) {
        std.log("----- HASH req.http.Accept-Encoding: " + req.http.Accept-Encoding);
    ...
    if(false && req.http.X-Varnish-Cache-Url) {
        std.log("----- HASH req.http.X-Varnish-Cache-Url: " + req.http.X-Varnish-Cache-Url);
    ...
    if (req.http.Host) {
        std.log("----- HASH req.http.Host: " + req.http.Host);
        hash_data(req.http.Host);
    } else {
        std.log("----- HASH server.ip: " + server.ip);
        hash_data(server.ip);
    }

    hash_data(req.http.Ssl-Offloaded);
    if (req.http.X-Normalized-User-Agent) {
        std.log("----- HASH req.http.X-Normalized-User-Agent: " + req.http.X-Normalized-User-Agent);
    ...
    if (req.http.X-Varnish-Store || req.http.X-Varnish-Currency) {
        std.log("----- HASH req.http.X-Varnish-Store: " + req.http.X-Varnish-Store);
        hash_data("s=" + req.http.X-Varnish-Store + "&c=" + req.http.X-Varnish-Currency);
    }

    if (req.http.X-Varnish-Esi-Access == "private" && req.http.Cookie ~ "frontend=") {
        std.log("----- HASH ESI" + regsub(req.http.Cookie, "^.*?frontend=([^;]*);*.*$", "\1"));
        hash_data(regsub(req.http.Cookie, "^.*?frontend=([^;]*);*.*$", "\1"));
    }

    if (req.http.X-Varnish-Esi-Access == "customer_group" && req.http.Cookie ~ "customer_group=") {
        std.log("----- HASH ESI" + regsub(req.http.Cookie, "^.*?customer_group=([^;]*);*.*$", "\1"));
...
sub vcl_fetch {
    ...
    if (true &&
    bereq.url ~ ".*\.(?:css|js|jpe?g|png|gif|ico|swf|svg)(?=\?|&|$)") {
...
sub vcl_deliver {
    ...
    std.log("----- STATE: [ deliver ] with url: ");
    std.log("URL: " + req.url);
    std.log("Cookie: " + req.http.Cookie);
